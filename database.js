...
async updateAdBalance(userId, amount) {
    await this.updateUser(userId, u => {
        u.adBalance = (u.adBalance || 0) + amount;
    });
}

async addAdTask(userId, task) {
    await this.updateUser(userId, u => {
        u.adTasks = u.adTasks || [];
        u.adTasks.push(task);
    });
...
async getAdTasks() {
    // Возвращаем все рекламные задания
    const tasks = [];
    const users = await this.getAllUsers();
    
    for (const user of users) {
        if (user.adTasks && user.adTasks.length > 0) {
            tasks.push(...user.adTasks);
        }
    }
...
async removeAdTask(userId, taskId) {
    await this.updateUser(userId, u => {
        if (u.adTasks) {
            u.adTasks = u.adTasks.filter(t => t.id !== taskId);
        }
    });
}
